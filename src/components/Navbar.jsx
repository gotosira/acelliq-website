...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-bg-dark/80 backdrop-blur-md border-b border-border-subtle/50">
      <div className="max-w-[1440px] mx-auto flex items-center justify-between px-8 md:px-20 py-5">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link
            to="/contact"
            className="bg-accent-lime text-bg-dark text-sm font-semibold px-6 py-2.5 rounded-3xl no-underline hover:brightness-110 transition"
          >
            Contact us
          </Link>
        </div>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden bg-bg-dark border-t border-border-subtle px-8 py-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              ...
              className={`text-base font-medium no-underline ${
                pathname === link.path ? 'text-text-white' : 'text-text-muted'
              }`}
            >
              {link.label}
            </Link>
